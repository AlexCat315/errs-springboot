...
import { send_code } from '../../../net/account/forgot_password';
...
<template>
  ...
  <form class="form">
    ...
    <label>
      <input v-model="forgotFrom.email" required placeholder="" type="email" class="input">
      <span>电子邮箱</span>
    </label>

    <div class="code-container">
      <label style="flex:1">
        <input v-model="forgotFrom.code" required placeholder="" type="text" class="input" maxlength="6">
        <span>验证码</span>
      </label>
      <button class="code-btn" type="button" @click="sendCode" :disabled="isSending">
        {{ isSending ? `${countdown}s` : '获取验证码' }}
      </button>
    </div>

    <label>
      <input v-model="forgotFrom.password" required placeholder="" type="password" class="input">
      <span>新密码</span>
    </label>

    <label>
      <input v-model="forgotFrom.password_confirm" required placeholder="" type="password" class="input">
      <span>重复新密码</span>
    </label>

    <button class="submit">重置密码</button>
    <span @click="showLogin()">
      <p class="signin">回忆起密码? <a style="cursor: pointer;">登录</a></p>
    </span>
  </form>
</template>
