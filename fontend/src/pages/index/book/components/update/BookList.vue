...
const fetchBooks = (page = 1, size = 10) => {
    get_book_info_all(
        page,
        size,
        (data: any) => {
            ...
            if (currentPage.value === 1) {
                ...
                const uniqueBooks = [...booksList.value, ...newBooks].filter(
                    (book, index, self) =>
                        self.findIndex((b) => b.id === book.id) === index
                );
                booksList.value = uniqueBooks;
            }
        },
        () => { }
    );
...
<template>
    <div class="container">
        <div ref="scrollContainer" @scroll="handleScroll" class="gard">
            <div v-for="(item, index) in booksList" :key="item.id" class="card">
                <div style="display: flex">
                    <p class="rank-index">
                        {{ index + 1 }}
                    </p>
                    <img alt="cover" class="movies-img" :src="item.img" />
                    <div style="width: 90%">
                        <!-- 标题和基本信息 -->
                        ...
                        <div style="margin-top: 5px" class="flex-center--y">
                            ...
                            <div style="margin-left: 20px; color: #666;display: flex;">
                                <span style="font-size: 13px;">推荐度：{{ item.recommend }}%</span>
                            </div>
                        </div>

                        <!-- 其他信息 -->
                        <div style="margin-top: 1px; color: #888;font-size: 14px;margin-left: 15px;">
                            <span>评价人数：{{ item.users }}</span>
                        </div>
