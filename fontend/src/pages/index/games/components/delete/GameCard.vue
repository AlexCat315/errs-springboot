...
onMounted(() => {
    get_game_info_by_seach(
        ...
        (data: any) => {
            ...
            gamesList.value = data.slice(0, 6); // 赋值返回的游戏数组
        },
        () => {},
    );
});
...
const fetchGames = (page = 1, size = 10) => {
    get_game_info_by_seach(
        ...
        size,
        (data: any[]) => {
            const newGames = data.map((game: any) => {
                game.gameUsers = formatNumber(game.gameUsers);
                game.gameScore = parseFloat(game.gameScore.toFixed(1));
                return game;
            });
...
.gard::-webkit-scrollbar-thumb {
    background: transparent; /* 背景透明，实际上隐藏了滑块 */
}
